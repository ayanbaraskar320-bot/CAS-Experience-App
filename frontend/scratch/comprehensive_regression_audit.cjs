...
const forbiddenAnchorMatches = [
  ...
  'path-nonprofit',
  'path-individual'
].filter(alias => audienceRoutingCode.includes(`'${alias}'`));
check(
  '1.3',
  'Institutional programs NEVER act as base participant entry points',
  forbiddenAnchorMatches.length === 0,
  `0 forbidden institutional anchorIds in AUDIENCE_PATHS / PARTICIPANT_ENTRY_POINTS`,
